...
class WC_Shipping_Free_Shipping extends WC_Shipping_Method {
	...
	public function is_available( $package ) {
		...
		echo ("<h2> Entrou em is avaiable</h2><br>");
		...
		if ( in_array( $this->requires, array( 'min_amount', 'either', 'both' ), true ) ) {
			...
			// aqui começam as modificações do Henry
			$valor_minimo = $this->min_amount;
			settype($valor_minimo, "double");

			if($total >= $valor_minimo){
				echo "<br> Mudança Aprovada !!";
				$has_met_min_amount = true;
			}
			//aqui terminam as modificações. 

			if ( $total >= $this->min_amount ) {
			//eu poderia somente tipar $this->min_amount, mas não sei se em algum lugar do código ele precisa ser uma string, da forma que eu fiz n quebra nada pelo menos

				echo "<br> Deu verdadeiro no valor mínimo";
				$has_met_min_amount = true;
			}
		}
